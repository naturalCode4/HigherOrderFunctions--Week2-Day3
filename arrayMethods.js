...
const evenNumbers2 = mixedNumbers.filter(getEven)/* Provide Your Callback Here */
...
const tax = initialPrice => initialPrice * 1.07
...
// CODE HERE
const populationCallback = (accumulatedVal, nextVal) => {
  return accumulatedVal + nextVal}
const totalPopulation = populations.reduce(populationCallback)
...
// const filterStrong = (monster) => {
//   return monster.CP > 200
// }
// const myStrong = monstersInYourPocket.filter(filterStrong)

// // console.log(myStrong)

//This code finds your strongest pokemon
let currentStrongest = monstersInYourPocket[0]

const findStrongestCallback = (monster) => {
  if (monster.CP > currentStrongest.CP) {
    currentStrongest = monster
  }
  return currentStrongest
}

const findStrongest = () => {
  monstersInYourPocket.forEach(findStrongestCallback)
  return 'Your strongest pokemon is ' + currentStrongest.monster + ' with a CP of ' + currentStrongest.CP
}
...
}, 0) //notice we are giving starting value of 0, or else it will take first value as starting value

const bobsTotal2 = purchases.filter(purchase => purchase.owner === 'Bob').reduce((acc, purchase) => {
  return acc + purchase.price
}, 0)

// console.log(bobsTotal)
// console.log(bobsTotal2)